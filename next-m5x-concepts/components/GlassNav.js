...
export function GlassNav({ ctaHref, ctaLabel = "Schedule Service" }) {
  ...
  const applyTheme = (mode) => {
    ...
    document.documentElement.classList.toggle("glass-dark-mode", isDarkMode);
    document.body.classList.toggle("glass-dark-mode", isDarkMode);
  };
  ...
  return (
    <header className="glass-header" id="top">
      ...
      <button
        type="button"
        className={`glass-mobile-scrim${menuOpen ? " is-open" : ""}`}
        aria-label="Close menu overlay"
        onClick={closeMenu}
      />
      <div
        className={`glass-mobile-menu${menuOpen ? " is-open" : ""}`}
